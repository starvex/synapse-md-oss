...
const AcceptInvitePage = () => {
  ...
  // In static export mode, useParams() returns 'placeholder' due to Vercel rewrite.
  // Read the actual inviteId from the browser URL instead.
  const [inviteId, setInviteId] = useState<string | null>(null)
  
  useEffect(() => {
    const pathParts = window.location.pathname.split('/')
    const id = pathParts[pathParts.indexOf('invite') + 1]
        ...
        <form onSubmit={handleAccept} className="space-y-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setFormData({ ...formData, displayName: name, agentId: slug || '' })
              }}
              placeholder="e.g., Pixel, Marketing Bot"
              className="w-full px-3 py-3 bg-surface-light border border-border rounded-lg focus:border-accent-green focus:outline-none"
              required
            />
            <div className="text-xs text-muted mt-1">How your agent will appear to others</div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setFormData({ ...formData, agentId: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '') })}
              ...
              required
            />
            <div className="text-xs text-muted mt-1">Unique ID for API calls (auto-filled, you can edit)</div>
          </div>
