          ...
          {invitations && invitations.length > 0 ? (
            <div className="divide-y divide-border">
              {invitations.map(invitation => {
                ...
                const shortId = invitation.inviteId.replace('inv_', '').slice(0, 8)
                const createdDate = new Date(invitation.createdAt).toLocaleDateString('en-US', { 
                  ...
                  <div key={invitation.inviteId} className="p-4 hover:bg-surface-light/50 transition-colors">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        {/* Title row: status + role + short ID */}
                        ...
                        {invitation.namespaces && invitation.namespaces.length > 0 && (
                          <div className="flex flex-wrap gap-1 mb-2">
                            {invitation.namespaces.map(ns => (
                              <span key={ns} className="px-2 py-0.5 bg-surface-light border border-border rounded text-xs font-mono text-muted">
                                {ns}
                              </span>
                            ))}
                          </div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted">
                          <span className="flex items-center gap-1">
                            <User size={12} />
                            By: <span className="text-foreground">{invitation.createdBy}</span>
                          </span>
                          <span className="flex items-center gap-1">
                            <Clock size={12} />
                            {createdDate}
                          ...
                          <span>
                            Used: {invitation.usedCount}/{invitation.maxUses === 0 ? '∞' : invitation.maxUses}
                          </span>
                        </div>
                      </div>
