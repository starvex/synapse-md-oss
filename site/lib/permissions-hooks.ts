...
async function getWorkspaceId(apiKey: string): Promise<string> {
  // Use /auth/me endpoint to get workspace ID (works even with empty workspace)
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Invitation[]> => {
      ...
      return (data.invitations || []).map((inv: any) => ({
        ...
        maxUses: inv.maxUses,
        usedCount: inv.usedCount || inv.uses || 0,
        status: inv.status,
        createdBy: inv.createdBy,
        createdAt: inv.createdAt,
        inviteUrl: inv.inviteUrl || `https://synapse-md.vercel.app/invite/${inv.inviteId}`
      }))
    },
    enabled: !!auth,
    staleTime: 30000,
