...
class SynapseClient {
    ...
    async request(method, path, body, extraHeaders) {
        ...
        if (!res.ok) {
            ...
            (0, errors_js_1.throwSynapseError)(res.status, errBody);
        }
        // 204 No Content
        if (res.status === 204) {
            return undefined;
     ...
     * ```
     */
    async write(namespace, content, options = {}) {
        const body = {
            namespace,
            content,
        };
        if (this.config.agentId)
            body['from_agent'] = this.config.agentId;
        if (options.priority)
    ...
    async list() {
        ...
        const res = await this.get('/entries', { limit: 1000 });
        const map = new Map();
     ...
     */
    async getEntry(id) {
        const res = await this.get(`/entries/${encodeURIComponent(id)}`);
        return res.entry;
    }
    /**
    ...
    async deleteEntry(id) {
        await this.request('DELETE', `/entries/${encodeURIComponent(id)}`);
    }
    // ─── Agent Management ────────────────────────────────────────────────────────
    /**
     ...
     * ```
     */
    async registerAgent(id, options = {}) {
        const wsId = await this.workspaceId();
        return this.request('POST', `/workspaces/${wsId}/agents`, {
            agentId: id,
    ...
    async createWebhook(url, options = {}) {
        const wsId = await this.workspaceId();
        return this.request('POST', `/workspaces/${wsId}/webhooks`, {
            url,
    ...
    async listWebhooks() {
        const wsId = await this.workspaceId();
        const res = await this.get(`/workspaces/${wsId}/webhooks`);
        return res.webhooks;
    }
    /**
    ...
    async deleteWebhook(id) {
        const wsId = await this.workspaceId();
        await this.request('DELETE', `/workspaces/${wsId}/webhooks/${encodeURIComponent(id)}`);
    }
    /**
    ...
    async testWebhook(id) {
        const wsId = await this.workspaceId();
        return this.request('POST', `/workspaces/${wsId}/webhooks/${encodeURIComponent(id)}/test`);
    }
    // ─── Permissions ─────────────────────────────────────────────────────────────
    /**
     ...
     * ```
     */
    async setPermission(agentId, namespace, permission) {
        const wsId = await this.workspaceId();
        await this.request('POST', `/workspaces/${wsId}/permissions`, {
            agentId,
            namespace,
            permission,
        });
    ...
    async listPermissions() {
        const wsId = await this.workspaceId();
        const res = await this.get(`/workspaces/${wsId}/permissions`);
    ...
    async freeze() {
        const wsId = await this.workspaceId();
        await this.request('POST', `/workspaces/${wsId}/freeze`, { frozen: true });
    }
    /**
    ...
    async unfreeze() {
        const wsId = await this.workspaceId();
        await this.request('POST', `/workspaces/${wsId}/freeze`, { frozen: false });
    }
    /**
    ...
    async auditLog(options = {}) {
        const res = await this.get('/audit', {
            limit: options.limit,
        });
        return res.events;
    }
     ...
     * ```
     */
    async bridge(toWorkspaceId, namespace, content) {
        const wsId = await this.workspaceId();
        const res = await this.request('POST', '/bridge', {
            from_workspace: wsId,
     ...
     * ```
     */
    static generateFingerprint(gatewayId, agentId, secret) {
